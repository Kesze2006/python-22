...
class forgato:
    ...
    def __init__(self,canvas,A,K,O,S):
        self.canvas=canvas
        self.A=A
        self.K=K
        self.O=O
    ...
    def rajzol(self):
        ...
        for i,betu in enumerate(self.A):
            betu=self.eltol(betu,-self.kozep[0],-self.kozep[1])
            
            betu=self.forgat(betu,self.szog)
            
            betu=self.eltol(betu,0,-90)
            
            betu=self.eltol(betu,self.kozep[0],self.kozep[1])
            
            self.canvas.create_line(betu,fill=self.szinek[i],width=5)

        for i,betu in enumerate(self.K):
            betu=self.eltol(betu,-self.kozep[0],-self.kozep[1])
            
            betu=self.forgat(betu,self.szog)
            
            betu=self.eltol(betu,-240,560)
            
            betu=self.eltol(betu,self.kozep[0],self.kozep[1])
            
            self.canvas.create_line(betu,fill=self.szinek[i],width=5)

        for i,betu in enumerate(self.O):
            betu=self.eltol(betu,-self.kozep[0],-self.kozep[1])
            
            betu=self.forgat(betu,self.szog)
            
            betu=self.eltol(betu,515,-100)
            
            betu=self.eltol(betu,self.kozep[0],self.kozep[1])
            
            self.canvas.create_line(betu,fill=self.szinek[i],width=5)

        for i,betu in enumerate(self.S):
            betu=self.eltol(betu,-self.kozep[0],-self.kozep[1])
            
            betu=self.forgat(betu,self.szog)
            
            betu=self.eltol(betu,260,560)
